...
public class Class1
{
    ...
    public class FloatingPointSequence : IEnumerable<double>
    {
        ...
        private Queue<double> _recentNumbers = new Queue<double>();
        ...
        public IEnumerator<double> GetEnumerator()
        {
            ...
            while (true)
            {
                ...
                _recentNumbers.Enqueue(number);
                if (_recentNumbers.Count > WindowSize)
                {
                    _recentNumbers.Dequeue();
                }
                if (number <= 0.5)
                {
                    consecutiveLowCount++;
                ...
                // bool invalidSequenceCondition = IsInvalidSequence();
                if (consecutiveLowCount >= 3 || IsInvalidSequence())
                {
                    consecutiveLowCount = 0;
        ...
        private bool IsInvalidSequence()
    {
        int lowCount = _recentNumbers.Count(n => n <= 0.5);
        return lowCount > _recentNumbers.Count / 2;
    }
